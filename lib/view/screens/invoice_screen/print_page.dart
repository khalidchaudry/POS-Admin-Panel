                      ...
                      children: <Widget>[
                        ...
                        OutlinedButton(
                          onPressed:  _connected?() async {
                            ...
                            // list.add(LineText(type: LineText.TYPE_TEXT, content: '**********************************************', weight: 1, align: LineText.ALIGN_CENTER,linefeed: 1));
                            // list.add(LineText(linefeed: 1));
                            ByteData data = await rootBundle.load(Images.phoneField);
                            List<int> imageBytes = data.buffer.asUint8List(data.offsetInBytes, data.lengthInBytes);
                            String base64Image = base64Encode(imageBytes);
                            list.add(LineText(type: LineText.TYPE_IMAGE, content: base64Image, align: LineText.ALIGN_CENTER));
                            await bluetoothPrint.printReceipt(config, list);
                          }:null,
                          child: const Text('Print Invoice(POS)'),
                        ),
                        OutlinedButton(
                          onPressed:  _connected?() async {
                            Map<String, dynamic> config = {};
                            ...
                            // List<LineText> list1 = [];
                            // ByteData data = await rootBundle.load(Images.noProduct);
                            // List<int> imageBytes = data.buffer.asUint8List(data.offsetInBytes, data.lengthInBytes);
                            // String base64Image = base64Encode(imageBytes);
